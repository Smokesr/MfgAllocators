...
/*
    void* BlockAllocator::allocate(const size_t& size, void* nextTo) {
                    if(nextTo = nullptr) {
                    ...
                    size_t nextToSize = *((size_t*) (nextTo - sizeof(size_t)));
                    void* nextToEnd = (void*) (nextTo + nextToSize - sizeof(size_t));

                    Block* block = mBlocks;
                    Block* prev = nullptr;

                    while(block != nullptr) {
                        if((void*) block == nextToEnd) {
                            break;
                        }
                        prev = block;
                        block = block->next;
                    ...
                    if(prev != nullptr) {
                        if(block != nullptr) {
                            if(block->size - size < sizeof(Block) {
                                ...
                                ///TODO: lefoglalunk egy mind2nek elég helyet és átmásoljuk az eddigit majd visszatérünk
                            }
                        }
                        else {
                            ///TODO: lefoglalunk egy mind2nek elég helyet és átmásoljuk az eddigit majd visszatérünk
                    ...
                    while(block != nullptr) {
                        if(block->size >= newSize && block->size < bestFit->size && block->size - newSize >= sizeof(Block)) {
                            bestFit = block;
                            bestFitPrev = prev;
                        }
                        prev = block;
